...
**목표**: 메뉴 [5] 생산라인 조회 기능을 완성하고 시간 기반 자동 완료 처리를 구현한다.

**선행 조건**: Phase 4 완료 (생산 큐에 작업이 쌓여야 함)

### 작업 항목

- `ProductionJob`에 `startedAt` (시작 시각) 및 `avgProductionTime` 추가
- `ProductionQueue.enqueue()`: 큐가 비어있을 때 첫 작업 진입 시 `startedAt` 기록
- `ProductionQueue.dequeue()`: 완료 후 다음 작업에 `startedAt` 기록 (Clock 주입)
- **시간 기반 자동 완료**: 조회 시 경과 시간 ≥ 총 생산 시간이면 자동으로 재고 증가 + `CONFIRMED` 전환
- View — 진행률(%), 완료 예정 시각, 대기 큐 예상 완료 시각 출력

### 완료 기준

- 생산라인 조회 시 현재 작업의 진행률(%)과 완료 예정 시각이 표시된다.
- 경과 시간이 총 생산 시간을 초과하면 **수동 입력 없이** 자동으로 `CONFIRMED` 전환된다.
