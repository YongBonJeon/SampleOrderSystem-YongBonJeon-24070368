...
#### STEP 1: Plan.md 작성 (사람 검토 필수)

테스트를 작성하기 전에, 무엇을 테스트할지 계획을 먼저 `Plan.md`에 작성한다.
...
**⛔ Plan.md 작성 후 반드시 멈춰라.**

```
Plan.md를 작성했습니다. 검토 후 진행을 허락해 주세요.
```
...
승인을 받은 뒤, Plan.md의 설계대로 테스트 하나를 작성한다.
...
사람의 명시적 지시 없이는 다음 사이클의 Plan.md를 작성하지 않는다.
...
사람이 다음 동작을 지시하면 RED 단계(Plan.md)부터 다시 시작한다.
...
| "Plan.md 없이 바로 테스트 쓰면 되지 않나" | 계획 없는 테스트는 범위를 벗어나기 쉽다. 사람의 눈으로 먼저 검토한다. |
...
- 테스트를 "나중에" 추가
- Plan.md 없이 테스트 작성
- 사람의 승인 없이 Plan.md → 테스트 작성으로 넘어감
...
| 문제 | 해결 |
|------|------|
| 어떻게 테스트할지 모르겠다 | Plan.md에 먼저 적어보라. assertion부터 작성하라. 사람에게 물어보라. |
...
| Plan.md 범위가 너무 크다 | 한 사이클 = 하나의 동작. 범위를 줄여라. |
...
RED 단계 → Plan.md 승인 없이 테스트를 작성하지 않는다
